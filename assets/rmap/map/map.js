...
---
// Constants
const MAP_PATH =  "/assets/rmap/data"; //  '{{ site.url }}/assets/rmap/data';
...
const nhs_scotland_map_path = data_path.concat('/', NHS_SCOTLAND_MAP);
const site_data_path = data_path.concat('/', SITE_DATA_PATH);
const metadata_path = data_path.concat('/', METADATA_PATH);
const england_meta_areas_map_path = data_path.concat('/', ENGLAND_META_AREA_MAP);
const topojson_path  = data_path.concat('/', TOPOJSON_PATH);
...
const loadNHSScotland = d3.csv(nhs_scotland_map_path).then(data => data.forEach(d => {
...
const loadEnglandMetaAreas = d3.csv(england_meta_areas_map_path).then(data => data.forEach(d => {
